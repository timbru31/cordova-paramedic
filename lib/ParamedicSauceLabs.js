...
class ParamedicSauceLabs {
    ...
    displaySauceDetails (buildName) {
        ...
        return sauce.listJobs(this.config.getSauceUser()).then(jobs => {
            let found = false;
            for (let job in jobs) {
                if (
                    jobs.hasOwnProperty(job) &&
                jobs[job].name &&
                jobs[job].name.indexOf(buildName) === 0
                ) {
            ...
            if (!found) {
                logger.warn(
                    'Can not find saucelabs job. Logs and video will be unavailable.'
                );
            }
    ...
    getSauceCaps () {
        ...
        if (this.isBrowser) {
            caps['sauce:options'] = sauceLabsOptions;
        } else {
            caps = sauceLabsOptions;
        }
        ...
        case utilities.BROWSER:
            ...
            caps.browserVersion = this.config.getSaucePlatformVersion() || '85.0';
            caps.platformName = caps.browserName.indexOf('Edge') > 0 ? 'Windows 10' : 'macOS 10.15';
            ...
            if (!caps['sauce:options'] && process.env[utilities.SAUCE_TUNNEL_ID_ENV_VAR]) {
                caps['sauce:options'].tunnelIdentifier = process.env[utilities.SAUCE_TUNNEL_ID_ENV_VAR];
            } else if (!caps['sauce:options']) {
