...
function getSimulatorModelId(cli, target) {
    ...
    logger.info('found:');
    logger.info('findSimResult: ' + findSimResult);
    logger.info('    ' + findSimResult.output);
    logger.info('    ' + findSimResult.output.split(/\r\n|\r|\n/).length);

    if(findSimResult.output.split(/\r\n|\r|\n/).length == 1) {
        logger.error('Could not find an appropriate simulator.');

        var unfilteredFindSimCommand = cli + ' run --list --emulator' + module.exports.PARAMEDIC_COMMON_CLI_ARGS;
        logger.info('running:');
